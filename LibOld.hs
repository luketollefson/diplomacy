...
permutationToDisjointCycles (Permutation perm) = res where
  ...
  -- high level overview
  -- 1. we get a mutable array tag, it has false
  -- 2. the worker will build up a cycle, changing all the values it
  -- sees into True
  -- 3. the next tag (k+1) will scan left to right to find the next 
  -- False index
  -- 4. unfoldM will perform this on every index, starting with 1

  res = runST $ do
    -- gets array with False of indicies (1,n)
    tag <- newArray (1,n) False
    -- unfoldM :: (b -> m (a, Maybe b)) -> b -> m [a]
    cycles <- unfoldM (step tag) 1
    -- filter out the trial cycles
    return (DisjointCycles $ filter f cycles)

  -- the state in all these are the tag array, as it is 
  -- written and read from

  step :: STUArray s Int Bool -> Int -> ST s ([Int],Maybe Int)
  step tag k = do
    cyc <- worker tag k k [k] -- get the cycle of ints
    m <- next tag (k+1) -- get the maybe int of the next index
    return (reverse cyc, m) -- (reverse cycle, the Maybe Int thing)

  -- next and worker are not mutually recursive, so 
  -- they should be independenltly understood
  next :: STUArray s Int Bool -> Int -> ST s (Maybe Int)
  next tag k = if k > n
    then return Nothing -- it's bigger than the bounds
    else readArray tag k >>= \b -> if b -- b is the bool
      then next tag (k+1) -- go to the next index
      else return (Just k) -- if its false return Just k

  -- scan the array putting True on l
  -- This gets one cycle, the ST state is ???
  -- tag, the array
  -- k, the first indice
  -- l, the iterated indice
  -- cyc, elements in the cycle
  worker :: STUArray s Int Bool -> Int -> Int -> [Int] -> ST s [Int]
  worker tag k l cyc = do
    writeArray tag l True -- write True to indecie
    let m = perm ! l -- get the next element
    if m == k           -- if the next was the initial
      then return cyc
      else worker tag k m (m:cyc) -- recur
-}